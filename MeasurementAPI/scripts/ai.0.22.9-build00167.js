        ...
        var Sender = (function () {
            ...
            Sender.prototype.send = function (envelope) {
                ...
                try {
                    ...
                    var measureload = ApplicationInsights.Serializer.serialize(envelope);
                    if (this._getSizeInBytes(this._buffer) + measureload.length > this._config.maxBatchSizeInBytes()) {
                        this.triggerSend();
                    }
                    this._buffer.push(measureload);
            ...
            Sender.prototype._xhrSender = function (measureload, isAsync, countOfItemsInMeasureload) {
                var xhr = new XMLHttpRequest();
                xhr[ApplicationInsights.AjaxMonitor.DisabledPropertyName] = true;
                xhr.open("POST", this._config.endpointUrl(), isAsync);
                xhr.setRequestHeader("Content-type", "application/json");
                xhr.onreadystatechange = function () { return Sender._xhrReadyStateChange(xhr, measureload, countOfItemsInMeasureload); };
                xhr.onerror = function (event) { return Sender._onError(measureload, xhr.responseText || xhr.response || "", event); };
                xhr.send(measureload);
            };
            Sender.prototype._xdrSender = function (measureload, isAsync) {
                var xdr = new XDomainRequest();
                xdr.onload = function () { return Sender._xdrOnLoad(xdr, measureload); };
                xdr.onerror = function (event) { return Sender._onError(measureload, xdr.responseText || "", event); };
                xdr.open('POST', this._config.endpointUrl());
                xdr.send(measureload);
            };
            Sender._xhrReadyStateChange = function (xhr, measureload, countOfItemsInMeasureload) {
                if (xhr.readyState === 4) {
                    if ((xhr.status < 200 || xhr.status >= 300) && xhr.status !== 0) {
                        Sender._onError(measureload, xhr.responseText || xhr.response || "");
                    }
                    else {
                        Sender._onSuccess(measureload, countOfItemsInMeasureload);
                    }
                }
            };
            Sender._xdrOnLoad = function (xdr, measureload) {
                if (xdr && (xdr.responseText + "" === "200" || xdr.responseText === "")) {
                    Sender._onSuccess(measureload, 0);
                }
                else {
                    Sender._onError(measureload, xdr && xdr.responseText || "");
                }
            };
            Sender._onError = function (measureload, message, event) {
            ...
            Sender._onSuccess = function (measureload, countOfItemsInMeasureload) {
                ApplicationInsights.DataLossAnalyzer.decrementItemsQueued(countOfItemsInMeasureload);
            };
            return Sender;
        })();
        ...
        var AppInsights = (function () {
            ...
            AppInsights.prototype.trackAjax = function (id, absoluteUrl, pathName, totalTime, success, resultCode) {
                if (this.config.maxAjaxCallsPerView === -1 ||
                    ...
                    ApplicationInsights._InternalLogging.throwInternalUserActionable(ApplicationInsights.LoggingSeverity.CRITICAL, new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.USRACT_MaxAjaxPerPVExceeded, "Maximum ajax per page view limit reached, ajax monitoring is paused until the next trackPageView(). In substance to increase the limit set the maxAjaxCallsPerView configuration parameter."));
                }
